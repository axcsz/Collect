...
Update_Shell(){
	...
	sh_new_ver=$(wget --no-check-certificate -qO- "https://mirror.ghproxy.com/https://raw.githubusercontent.com/axcsz/Collect/master/Shadowrocket/ss-rust.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1)
	...
	if [[ ${sh_new_ver} != ${sh_ver} ]]; then
		...
		[[ -z "${yn}" ]] && yn="y"
		if [[ ${yn} == [Yy] ]]; then
			wget -O ss-rust.sh --no-check-certificate https://mirror.ghproxy.com/https://raw.githubusercontent.com/axcsz/Collect/master/Shadowrocket/ss-rust.sh && chmod +x ss-rust.sh
